...
async function main() {
  ...
  const url = process.env.NEON_DB_POSTGRES_URL_NON_POOLING ?? process.env.NEON_DB_POSTGRES_URL;
  if (!url) throw new Error('No DB URL');
  const sql = neon(url);

  const rows = await sql`SELECT id, slug FROM published_collections WHERE color IS NULL`;
