...
export function AdminClient({
  ...
  >(Object.fromEntries(balances.map((b) => [b.clerk_user_id, b.chatEnabled])));
  ...
  async function handleSeedUsers() {
    ...
    try {
      ...
      setSeedStatus(
        result.ok
          ? { ok: true, message: `Seeded ${result.upserted} users.` }
          : { ok: false, message: result.error },
      );
    } catch {
      setSeedStatus({ ok: false, message: 'Request failed' });
    } finally {
      setSeeding(false);
    }
  }
      ...
      <div className={styles.container}>
        ...
        <section className={styles.section}>
          ...
          <p className={styles.empty}>
            Seed the users table from Clerk for all existing accounts.
          </p>
          <button
            type="button"
          ...
          >
            {seeding ? 'Seeding…' : 'Seed users from Clerk'}
          </button>
          {seedStatus && (
            <p className={seedStatus.ok ? styles.success : styles.error}>
              {seedStatus.message}
            </p>
          )}
        </section>
