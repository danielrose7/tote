...
export function EditLinkDialog({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const updatedProductData = {
        ...
        imageUrl: values.imageUrl || block.productData.imageUrl,
      };
  ...
  useEffect(() => {
    if (block && productData) {
      // Backfill images array for products saved before gallery extraction
      if (productData.imageUrl && !productData.images) {
        block.$jazz.set('productData', {
          ...productData,
                ...
                <div className={styles.imagePicker}>
                  {productData.images.map((url: string) => (
                    <img
                      ...
                      src={url}
                      alt=""
                      className={`${styles.imagePickerThumb}${url === formik.values.imageUrl ? ` ${styles.imagePickerThumbSelected}` : ''}`}
                      onClick={() => formik.setFieldValue('imageUrl', url)}
                      onError={(e) => {
                        (e.target as HTMLImageElement).style.display = 'none';
                      }}
                    />
