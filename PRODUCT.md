...
- A collection stays private until the owner publishes it

### 2. Speed

- Fast page loads, instant interactions
- No spinners for common operations
- Collection data lives in Postgres; there is no local-first or offline
  store. Previously loaded collections stay readable in the session cache,
  but saving, syncing, and price refresh all need a network connection.
  Don't claim offline support in product or marketing copy.
...
3. **Degrade honestly** - Say what needs a connection instead of implying nothing does
