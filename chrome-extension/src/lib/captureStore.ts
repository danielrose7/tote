...
// The cache mirrors the last successful read from the API; the outbox
// preserves not-yet-accepted user intent across popup restarts.
...
const ACTIVE_USER_KEY = 'capture:activeUserId';

function indexKey(userId: string) {
  return `capture:index:${userId}`;
}

function outboxKey(userId: string) {
  return `capture:outbox:${userId}`;
}

function lastCollectionKey(userId: string) {
  return `capture:lastCollection:${userId}`;
}
