...
describe('SaveUI', () => {
  ...
  it('creates a collection and selects it', async () => {
    getTokenMock.mockResolvedValue('token');
    const createdId = '40000000-0000-4000-8000-000000000010';
    const fetchMock = vi.fn(async (url: unknown, init?: RequestInit) => {
      if (init?.method === 'POST') {
        expect(String(url)).toContain('/api/v2/collections');
        return new Response(
          JSON.stringify({ id: createdId, replayed: false }),
          { status: 201 },
        );
      }
      return new Response(JSON.stringify({ collections: COLLECTIONS }), {
    ...
    const openCreate = container.querySelector(
    ...
    await act(async () => {
      fireEvent.change(nameInput, { target: { value: 'Desk gear' } });
    });
    const confirm = container.querySelector(
      '.new-collection-confirm',
    ) as HTMLButtonElement;
    await act(async () => {
    ...
    await waitFor(() =>
      expect(
        (container.querySelector('#neon-collection') as HTMLSelectElement)
          .value,
      ).toBe(createdId),
    );
    expect(container.textContent).toContain('Desk gear');
    const postCall = fetchMock.mock.calls.find(
      (call) => (call[1] as RequestInit | undefined)?.method === 'POST',
    );
    expect(
      JSON.parse(String((postCall?.[1] as RequestInit).body)),
    ).toMatchObject({ name: 'Desk gear' });
  });

  it('creates a section on the selected collection and selects it', async () => {
    getTokenMock.mockResolvedValue('token');
    const createdId = '40000000-0000-4000-8000-000000000020';
    const fetchMock = vi.fn(async (url: unknown, init?: RequestInit) => {
      if (init?.method === 'POST') {
        expect(String(url)).toContain(
          `/api/v2/collections/${COLLECTIONS[0].id}/nodes`,
        );
        return new Response(
          JSON.stringify({ id: createdId, replayed: false }),
          { status: 201 },
    ...
    await renderSaveUI({});
    await waitFor(() =>
      expect(container.querySelector('#neon-section')).toBeTruthy(),
    );

    const addSection = container.querySelectorAll(
      '.add-collection-button',
    )[1] as HTMLButtonElement;
    expect(addSection.disabled).toBe(false);
    await act(async () => {
    ...
    await act(async () => {
      fireEvent.change(nameInput, { target: { value: 'Shelf' } });
    });
    await act(async () => {
      fireEvent.click(
        container.querySelector('.new-collection-confirm') as HTMLButtonElement,
      );
    });

    await waitFor(() =>
      expect(
        (container.querySelector('#neon-section') as HTMLSelectElement).value,
      ).toBe(createdId),
    );
    const postCall = fetchMock.mock.calls.find(
      (call) => (call[1] as RequestInit | undefined)?.method === 'POST',
    ...
    expect(
      JSON.parse(String((postCall?.[1] as RequestInit).body)),
    ).toMatchObject({ type: 'section', title: 'Shelf', parentId: null });
  });
