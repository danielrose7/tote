...
import {
  ...
  CaptureRequestError,
  createCaptureIds,
  // Aliased: the component defines its own createCollection/createSection
  // handlers, and an unaliased import would be shadowed by them.
  createCollection as createCollectionRequest,
  createSection as createSectionRequest,
...
export function SaveUI({
  ...
  const createSectionIds = useRef<CaptureIds | null>(null);
  ...
  const chooseCollection = (collectionId: string) => {
    ...
    setSelectedSection('');
    // Sections belong to a collection, so an open draft no longer applies.
    setShowNewSection(false);
    setNewSectionName('');
    createSectionIds.current = null;
  ...
  // Also online-only: sections are created on the collection, and the cached
  // index is updated so the new one survives a popup restart.
  const createSection = async () => {
    const name = newSectionName.trim();
    if (!name || !userId || !selectedCollection) return;
    setCreatingSection(true);
    setError(null);
    try {
      const token = await getToken();
      if (!token) throw new Error('Your Tote session is unavailable.');
      createSectionIds.current ??= createCaptureIds();
      const result = await createSectionRequest({
        token,
        collectionId: selectedCollection,
        ids: createSectionIds.current,
        name,
      });
      createSectionIds.current = null;
      const nextCollections = collections.map((collection) =>
        collection.id === selectedCollection &&
        !collection.sections.some((section) => section.id === result.id)
          ? {
              ...collection,
              sections: [...collection.sections, { id: result.id, name }],
            }
          : collection,
      );
      setCollections(nextCollections);
      await writeCachedIndex(userId, nextCollections);
      chooseSection(result.id);
      setShowNewSection(false);
      setNewSectionName('');
    } catch (sectionError) {
      setError(
      ...
      <div className="form-group">
        ...
        <div className="collection-row">
          <select
            ...
            disabled={saving || creatingSection || !selected?.sections.length}
          >
          ...
          <button
            ...
            disabled={
              saving || creatingSection || offline || !selectedCollection
            }
            title={
              offline
                ? 'Creating sections requires a connection'
                : 'Add section'
            }
          >
            {showNewSection ? '×' : '+'}
        ...
        {showNewSection && (
          <div className="new-collection-row">
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter' && newSectionName.trim())
                  void createSection();
                if (event.key === 'Escape') {
                  setShowNewSection(false);
                  setNewSectionName('');
