...
export function useCollections(): UseCollectionsResult {
  ...
  const setSelectedCollectionAndPersist = useCallback((id: string) => {
    setSelectedCollection(id);
    chrome.storage?.local?.set({ lastSelectedCollection: id }).catch(() => {});
  }, []);

  // Set default collection when collections load
  useEffect(() => {
    if (collections.length > 0 && !selectedCollection) {
      // Try restoring last selection from storage
      chrome.storage?.local
        ?.get('lastSelectedCollection')
        .then((result) => {
          const lastId = result?.lastSelectedCollection;
          if (lastId && collections.find((c) => c.$jazz.id === lastId)) {
            setSelectedCollection(lastId);
            return;
          }

          // Fall back to defaultBlockId, then first collection
        ...
        .catch(() => {
          // No storage access — fall back to defaults
          const defaultId = root?.defaultBlockId;
          if (defaultId) {
            const defaultCollection = collections.find(
              (c) => c.$jazz.id === defaultId,
  ...
  return {
    collections,
    selectedCollection,
    setSelectedCollection: setSelectedCollectionAndPersist,
    slots,
