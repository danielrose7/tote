/**
 * Safari JavaScript preprocessing for the Tote share extension.
 *
 * Runs in Safari's existing page context (no extra page load needed).
 * Logic is ported from mobile-app/src/lib/extractorScript.ts —
 * keep in sync when changing extraction logic.
 *
 * Key differences from extractorScript.ts:
 * - Results go to arguments.completionFunction() instead of ReactNativeWebView.postMessage
 * - Single-escaped regexes (not inside a template literal)
 */
    ...
    try {
      ...
      function extractPriceFromText(text) {
        ...
        for (var i = 0; i < symbols.length; i++) {
          if (priceStr.indexOf(symbols[i]) !== -1) {
            currency = CURRENCY_MAP[symbols[i]];
            break;
          }
        }
      ...
      function findProduct(data) {
        ...
        if (data['@type']) {
          var types = Array.isArray(data['@type'])
            ? data['@type']
            : [data['@type']];
          if (
            types.some(function (t) {
              return t === 'Product' || t === 'IndividualProduct';
            })
          )
            return { product: data, variantMatched: true };
          if (
            types.some(function (t) {
              return t === 'ProductGroup';
            })
          ) {
            if (Array.isArray(data.hasVariant)) {
              ...
              for (var j = 0; j < data.hasVariant.length; j++) {
                if (data.hasVariant[j].offers) {
                  withOffers = data.hasVariant[j];
                  break;
                }
              }
              if (withOffers)
      ...
      function productCanonicalUrl(product) {
        ...
        var offerUrl = Array.isArray(offers)
          ...
          try {
            ...
            if (match) {
              ...
              return {
                ...
                imageUrl: match.variantMatched
                  ? resolveUrl(extractImage(p.image))
                  : undefined,
      ...
      function scorePriceElement(el, h1Text) {
        ...
        if (heading && h1Text) {
          ...
          if (hText === h1Text) score += 50;
            ...
            hText &&
            (hText.indexOf(h1Text) !== -1 || h1Text.indexOf(hText) !== -1)
          )
            score += 25;
        }
        return score;
        ...
        for (var s = 0; s < salePriceSelectors.length; s++) {
          try {
            ...
            if (el && el.textContent) {
              var r = extractPriceFromText(el.textContent.trim());
              if (r) return r;
            }
          } catch (e) {}
        }
        ...
        if (itempropEl) {
          ...
          if (content) {
            ...
            return {
              price: content,
              currency:
                (currencyEl && currencyEl.getAttribute('content')) || 'USD',
            };
          }
        }
        var selectors = [
            ...
            for (var j = 0; j < els.length; j++) {
              ...
              if (parsed)
                candidates.push({
                  ...
                  score: scorePriceElement(elem, titleText),
                });
            }
          } catch (e) {}
        }
            ...
            if (seen.indexOf(sp) === -1) {
              ...
              if (p2)
                candidates.push({
                  ...
                  score: scorePriceElement(sp, titleText) - 5,
                });
            }
          }
        }
      ...
      function isExcludedImage(url, alt, image) {
        ...
        if (
          image &&
          (image.getAttribute('itemprop') || '').toLowerCase() === 'logo'
        )
          return true;
        return false;
      }
      ...
      arguments.completionFunction({
        title: title ? decodeHtmlEntities(title) : document.title || null,
        ...
        imageUrl:
          ...
          domImage ||
          null,
        price: (jsonLd && jsonLd.price) || domPrice.price || og.price || null,
        currency:
          (jsonLd && jsonLd.currency) ||
          domPrice.currency ||
          og.currency ||
          null,
        brand: (jsonLd && jsonLd.brand) || og.brand || null,
      });
    } catch (e) {
      // Return whatever we can from document.title on error
      arguments.completionFunction({
