...
import { JazzAccount, Block, SharedCollectionRef, SharedWithMeList } from "@tote/schema";
...
export function AcceptInviteSheet({ invite, onClose }: Props) {
  ...
  async function accept() {
    try {
      ...
      const block = await Block.load(invite.collectionId as `co_z${string}`, {});
    ...
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          {status === "accepting" && (
            <>
              <ActivityIndicator size="large" color="#6366f1" style={styles.icon} />
