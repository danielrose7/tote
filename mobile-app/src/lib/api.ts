...
export async function provisionApiKey(token: string): Promise<string> {
...
export async function fetchCaptureCollections(
  ...
  const data = await request<{ collections: CaptureCollection[] }>(
    '/api/v2/capture',
    token,
  );
...
export async function reorderNodes(
  ...
  await request(`/api/v2/collections/${collectionId}/nodes/reorder`, token, {
    method: 'POST',
